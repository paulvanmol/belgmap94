...
%let REGION_LABEL=Belgium Province;
...
PROC GREDUCE DATA=MAPSCSTM.ProvMap OUT=MAPSCSTM.ProvMap;
	ID NISCODE;
RUN;
...
data &PROVINCE_DATASET.;
 ...
 IF substr(left(niscode),1,1) in ('7','4','3','1') THEN DO;
  ...
  END;
  ELSE IF left(niscode) = '20001' THEN IDNAME=name_dut;
  ELSE IF left(niscode) = '20002' THEN IDNAME=name_fre;
